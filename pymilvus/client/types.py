...
try:
    from pandas.api.types import infer_dtype, is_array_like, is_float, is_list_like, is_scalar
except ImportError:
    # Fallback if pandas is not available
    def is_scalar(val: Any):
        return isinstance(val, (int, float, str, bytes, bool, np.generic))

    def is_list_like(val: Any):
        return isinstance(val, (list, tuple, np.ndarray))

    def is_array_like(val: Any):
        return isinstance(val, (list, tuple, np.ndarray))

    def infer_dtype(_: Any):
        return "unknown"
...
class PrivilegeGroupItem:
    ...
    def __repr__(self) -> str:
        return f"PrivilegeGroupItem: <privilege_group:{self._privilege_group}>, <privileges:{self._privileges}>"


# Added from orm/types.py
dtype_str_map = {
...
def is_float_datatype(data_type: DataType):
    return data_type in (DataType.FLOAT, DataType.DOUBLE)
