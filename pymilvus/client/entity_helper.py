...
def pack_field_value_to_field_data(
    ...
    """Pack a single field value into protobuf FieldData.
    
    Uses TypeHandler for type-specific packing logic.
    
    Note: vector_bytes_cache is preserved for backwards compatibility but
    TypeHandlers handle byte operations internally now.
    """
    from pymilvus.client.type_handlers import get_handler
    
    field_type = field_data.type
    handler = get_handler(field_type)
    handler.pack_value(field_value, field_data, field_info)
