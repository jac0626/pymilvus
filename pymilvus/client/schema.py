...
try:
    ...
    def is_list_like(x: Any):
        return isinstance(x, (list, tuple))

    def is_scalar(x: Any):
        return isinstance(x, (int, float, str, bytes, bool))
...
from .utils import convert_struct_fields_to_user_format
...
def check_is_row_based(data: Union[List[List], List[Dict], Dict, pd.DataFrame]) -> bool:
    valid_types = (pd.DataFrame, list, dict) if pd is not None else (list, dict)
    if not isinstance(data, valid_types):
        msg = (
            "The type of data should be list or pandas.DataFrame or dict"
            if pd
            else "The type of data should be list or dict"
        )
        raise DataTypeNotSupportException(message=msg)
...
def _check_insert_data(data: Union[List[List], pd.DataFrame]):
    valid_types = (pd.DataFrame, list) if pd is not None else (list,)
    if not isinstance(data, valid_types):
        msg = (
            "The type of data should be list or pandas.DataFrame"
            if pd
            else "The type of data should be list"
        )
        raise DataTypeNotSupportException(message=msg)
